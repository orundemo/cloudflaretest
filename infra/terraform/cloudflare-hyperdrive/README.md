...
│ (cloudflaretest-stage, cloudflaretest-prod)  │
└─────────────────────────────────────────────────────┘
```
...
- **`cloudflare_hyperdrive_config`**: Hyperdrive gateway resource
  - **Name**: `{namespacePrefix}cloudflaretest-{environment}` (e.g., `cloudflaretest-stage`)
...
| `orgName` | string | `cloudflaretest` | Organization name (for tags and secret paths) |
| `owner` | string | `cloudflaretest` | Repository owner (for tags) |
| `repo` | string | `cloudflaretest` | Repository name (for tags and secret paths) |
| `namespace` | string | `cloudflaretest` | Namespace (for labels and naming conventions) |
...
1. **`supabase` component** (same repo)
   ...
   - This component reads credentials from the secret path: `sourceplane/cloudflaretest/supabase/{environment}`
...
- **Secret path**: `sourceplane/cloudflaretest/supabase/{environment}`
...
aws secretsmanager get-secret-value --secret-id sourceplane/cloudflaretest/supabase/stage
```

### Validate Syntax

```bash
cd /path/to/cloudflaretest
