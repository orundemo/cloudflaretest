...
`cloudflaretest` organization and stores generated credentials in AWS Secrets
Manager.
...
|-------------|----------------------------|--------------|
| stage       | cloudflaretest-stage    | Provisioned  |
| prod        | cloudflaretest-prod     | Provisioned  |
| dev         | —                          | Not provisioned |
...
State is stored at:
```
s3://sourceplane-<env>/env/<env>/cloudflaretest/supabase/terraform.tfstate
```
...
```
sourceplane/cloudflaretest/supabase/stage
sourceplane/cloudflaretest/supabase/prod
```
