...
Your backend holds one Cloudflare Test API key and **zero GitHub credentials**.
...
const cloudflareTest = new CloudflareTest({
  baseUrl: process.env.CLOUDFLARETEST_API_URL!,
  token: process.env.CLOUDFLARETEST_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await cloudflareTest.integrations.issueGithubToken(ORG_ID, {
