...
`cloudflaretest` — first-class TypeScript CLI for the Cloudflare Test control
...
This package is internal to the `cloudflaretest` monorepo and is not
...
```
cloudflaretest login    [--api-url=URL] [--token=BEARER]
cloudflaretest logout
cloudflaretest whoami
```

Reads (Task 0100):

```
cloudflaretest org list
cloudflaretest org use <org-id>
cloudflaretest org members
cloudflaretest project list
```

Writes (Task 0101):

```
cloudflaretest org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
cloudflaretest project create <name> [--idempotency-key=KEY]
cloudflaretest env create <project-id> <name> [--idempotency-key=KEY]
cloudflaretest api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
cloudflaretest webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
cloudflaretest usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
cloudflaretest billing summary
cloudflaretest audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`cloudflaretest org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `cloudflaretest login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/cloudflaretest/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/cloudflaretest/config.json` (mode 0644, not a secret). Override
both via `CLOUDFLARETEST_CONFIG_DIR` (used by tests).
