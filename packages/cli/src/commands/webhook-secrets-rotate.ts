// `cloudflaretest webhook secrets rotate <endpointId>` — Task 0110.
//
...
export async function webhookSecretsRotateCommand(
  ...
  if (endpointId === undefined || endpointId.length === 0) {
    throw new UsageError(
      "usage: cloudflaretest webhook secrets rotate <endpointId> [--idempotency-key=KEY] [--output=human|json]",
    );
  }
