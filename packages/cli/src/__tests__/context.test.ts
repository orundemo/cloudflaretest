...
describe("resolveConfigDir", () => {
  it("respects CLOUDFLARETEST_CONFIG_DIR override", () => {
    const prev = process.env["CLOUDFLARETEST_CONFIG_DIR"];
    process.env["CLOUDFLARETEST_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["CLOUDFLARETEST_CONFIG_DIR"];
      else process.env["CLOUDFLARETEST_CONFIG_DIR"] = prev;
    }
  });
});
