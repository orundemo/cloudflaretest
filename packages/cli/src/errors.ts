// CLI error helpers. Translate `CloudflareTestError` subclasses (from
...
import { CloudflareTestError, UnauthenticatedError } from "@saas/sdk";

import { formatErrorJson, type OutputMode } from "./output/index.js";

export class MissingAuthError extends Error {
  constructor() {
    super("not logged in (run `cloudflaretest login`)");
...
export class MissingOrgContextError extends Error {
  constructor() {
    super("no active organization (run `cloudflaretest org use <id>`)");
...
export function formatCliError({ err, mode }: FormatErrorInput): FormattedError {
  ...
  if (err instanceof UnauthenticatedError) {
    return formatSdk(4, err, "token rejected — run `cloudflaretest login` to refresh", mode);
  }
  if (err instanceof CloudflareTestError) {
