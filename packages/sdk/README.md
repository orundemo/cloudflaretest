...
Cloudflare Test TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the Cloudflare Test control plane API.

## Install (workspace)

This package is internal to the `cloudflaretest` monorepo. Add it as a
...
```ts
import { CloudflareTest } from "@saas/sdk";

const client = new CloudflareTest({
  baseUrl: "https://api.tester.dev",
  auth: { kind: "bearer", token: process.env["CLOUDFLARETEST_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `CloudflareTestError`. Branch on
...
| `CloudflareTestError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `CloudflareTestError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new CloudflareTest({ fetch })`.
