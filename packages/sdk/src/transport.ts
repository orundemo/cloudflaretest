// Base HTTP transport for the CloudflareTest SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `CloudflareTestError`
...
export interface ClientOptions {
  /**
   * Base URL of the CloudflareTest api-edge worker, e.g.
   * `https://api.tester.dev`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `CloudflareTest` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "CloudflareTest SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "CloudflareTest SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
