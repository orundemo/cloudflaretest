...
import { CloudflareTest } from "../index.js";
import {
  BadRequestError,
  ConflictError,
  ForbiddenError,
...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new CloudflareTest({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => CloudflareTestError]> = [
  ...
  it("forward-compat: unknown error code → base CloudflareTestError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(CloudflareTestError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as CloudflareTestError).code).toBe("future_quota_exceeded");
    }
  });
